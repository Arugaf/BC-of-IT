...
namespace Лаб_4._1
{
    public partial class Form1 : Form
    {
        ...
        /// </summary>
        //получает в качестве параметра объект класса ParallelSearchThreadParam, осуществляет перебор всех слов в массиве для поиска, который был передан данному потоку
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            timer.Stop();
                ...
                this.listBoxResult.Items.Clear();
                //Вывод результатов поиска 
                foreach (var x in Result) { string temp = x.word + "(расстояние=" + x.dist.ToString() + " поток=" + x.ThreadNum.ToString() + ")";
                    this.listBoxResult.Items.Add(temp); }
                //Окончание обновления списка результатов 
                this.listBoxResult.EndUpdate(); }
            else { MessageBox.Show("Необходимо выбрать файл и ввести слово для поиска");
